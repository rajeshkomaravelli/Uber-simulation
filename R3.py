...
class uber:
    ...
    def rearrange(self,a):
        ...
        try:
            ...
            raise  e
    def generate_random_reservations(self, num_reservations):
        reservations = []
        for _ in range(num_reservations):
            pickup = random.randint(0, self.b - 1)
            dropoff = random.randint(0, self.b - 1)
            while pickup == dropoff:
                dropoff = random.randint(0, self.b - 1)
            reservations.append([pickup, dropoff])
        return reservations   
    def update(self):
        try:
    ...
    def TakeRideRequests(self, G):
        """
        This function takes G as input and checks if a new request is made. If made, add it to its near vehicle list and rearrange priorities of the vehicle.
        """
        try: 
            # Generate a random number of reservations per hour within the specified range
            reservations_per_hour = random.randint(450, 600)
            # Calculate the reservations per tick based on the total clock ticks
            reservations_per_tick = reservations_per_hour / self.total_clock_ticks

            # Iterate through each simulation tick
            for tick in range(1, self.total_clock_ticks + 1):
                # Check if a reservation should be generated for this tick
                if random.random() < reservations_per_tick:
                    cust_id = "customer" + str(self.count)
                    self.count = self.count + 1
                    # Generate a random pickup node
                    pick_up = random.randint(0, self.n - 1)
                    # Generate a random drop node
                    drop = random.randint(0, self.n - 1)
                    # Add the reservation to the list
                    self.cust_list.append([pick_up, drop])

                    # Find the nearest vehicle and assign the reservation to it
            ...
            # Call the update function to ensure vehicle positions and priorities are updated
            self.update()
...
def __init__(self):
    try:
        self.b = 200
        self.d = 30   # Number of vehicles
        self.e = 5    # Vehicle capacity
        self.f = 24   # Total number of clock ticks (hours)

        # Initialize simulation state
        self.no_of_trips = 0
        self.count = 1
        self.cus_dir = {}
        self.cust_list = []  # List to store ride requests
